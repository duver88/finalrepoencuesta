...
@section('title', 'Tokens Sospechosos - ' . $group->name)
...
<div class="container-fluid px-0">
    ...
    <div class="mb-4 d-flex justify-content-between align-items-start">
        <div>
            <h1 class="h2 fw-bold mb-1" style="color: #1e293b;">
                <i class="bi bi-shield-exclamation text-warning"></i> Análisis de Tokens Sospechosos (Grupo)
            </h1>
            <p class="text-muted mb-0">{{ $group->name }}</p>
            <small class="text-muted">Generado: {{ now()->format('d/m/Y H:i:s') }}</small>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('admin.survey-groups.suspicious-tokens.export', $group) }}" class="btn btn-outline-success">
                <i class="bi bi-file-earmark-spreadsheet"></i> Exportar CSV Sospechosos
            </a>
            <a href="{{ route('admin.survey-groups.suspicious-tokens.export-clean', $group) }}" class="btn btn-success">
                <i class="bi bi-file-earmark-check"></i> Exportar CSV Limpio (Sin Sospechosos)
            </a>
            <a href="{{ route('admin.survey-groups.report.export-excel', $group) }}" class="btn btn-outline-primary">
                <i class="bi bi-file-earmark-excel"></i> Exportar Todos los Votos
            </a>
            <a href="{{ route('admin.survey-groups.report', $group) }}" class="btn btn-outline-secondary">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            @foreach($suspiciousData['same_minute'] as $index => $group_data)
            <div class="mb-4 p-3 border rounded bg-light">
                <h6 class="fw-bold text-danger">
                    <i class="bi bi-exclamation-triangle-fill"></i>
                    Grupo {{ $index + 1 }}: {{ $group_data['timestamp'] }} ({{ $group_data['count'] }} tokens)
                </h6>
                <div class="table-responsive mt-3">
                    <table class="table table-sm table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="small">{{ $tokenData['survey_title'] ?? 'N/A' }}</td>
                                <td><code class="small">{{ substr($tokenData['token'], 0, 10) }}...</code></td>
                                <td class="small">{{ $tokenData['used_at'] }}</td>
                                <td class="small">{{ substr($tokenData['fingerprint'], 0, 8) }}...</td>
    ...
    <!-- Tokens con 1 y 2 Minutos de Diferencia - Versión Compacta -->
    @if(count($suspiciousData['one_minute_diff']) > 0 || count($suspiciousData['two_minute_diff']) > 0)
    <div class="card mb-4">
        <div class="card-header bg-info bg-opacity-10">
            <h5 class="mb-0 fw-bold">
                <i class="bi bi-stopwatch-fill text-info"></i>
                Tokens con 1-2 Minutos de Diferencia
            </h5>
        </div>
        <div class="card-body">
            <!-- 1 Minuto -->
            @foreach($suspiciousData['one_minute_diff'] as $index => $group_data)
            <div class="mb-4 p-3 border rounded bg-light">
                <h6 class="fw-bold text-info">
                    <i class="bi bi-arrow-left-right"></i>
                    1 Min Diff - Grupo {{ $index + 1 }}: {{ $group_data['timestamp_1'] }} ↔ {{ $group_data['timestamp_2'] }}
                </h6>
                <div class="table-responsive mt-3">
                    <table class="table table-sm table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="small">{{ $group_data['timestamp_1'] }}</td>
                                <td class="small">{{ $tokenData['survey_title'] ?? 'N/A' }}</td>
                                <td><code class="small">{{ substr($tokenData['token'], 0, 10) }}...</code></td>
                                <td class="small fw-bold">{{ $tokenData['votes'][0]['answer'] ?? 'N/A' }}</td>
                                <td class="small fw-bold">{{ $tokenData['votes'][1]['answer'] ?? 'N/A' }}</td>
                                <td>
                                    <span class="badge bg-{{ $tokenData['votes'][0]['status'] == 'approved' ? 'success' : 'warning' }}">
                                        {{ ucfirst($tokenData['votes'][0]['status'] ?? 'N/A') }}
                                    </span>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td class="small">{{ $group_data['timestamp_2'] }}</td>
                                <td class="small">{{ $tokenData['survey_title'] ?? 'N/A' }}</td>
                                <td><code class="small">{{ substr($tokenData['token'], 0, 10) }}...</code></td>
                                <td class="small fw-bold">{{ $tokenData['votes'][0]['answer'] ?? 'N/A' }}</td>
                                <td class="small fw-bold">{{ $tokenData['votes'][1]['answer'] ?? 'N/A' }}</td>
            ...
            <!-- 2 Minutos -->
            @foreach($suspiciousData['two_minute_diff'] as $index => $group_data)
            <div class="mb-4 p-3 border rounded bg-light">
                <h6 class="fw-bold text-secondary">
                    <i class="bi bi-arrow-left-right"></i>
                    2 Min Diff - Grupo {{ $index + 1 }}: {{ $group_data['timestamp_1'] }} ↔ {{ $group_data['timestamp_2'] }}
                </h6>
                <div class="table-responsive mt-3">
                    <table class="table table-sm table-bordered">
    ...
    @else
    <div class="alert alert-success">
        <i class="bi bi-check-circle-fill"></i> No se encontraron tokens con 1-2 minutos de diferencia.
    </div>
    @endif
</div>
